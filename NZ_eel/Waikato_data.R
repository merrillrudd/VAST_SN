...
# obslen <- readRDS(file.path(data_dir, "Waikato_observations_lengthcat.rds"))
...
saveRDS(obs_all, file.path(data_dir2, "Waikato_observations_downstreamOnly.rds"))
saveRDS(network_all, file.path(data_dir2, "Waikato_network_downstreamOnly.rds"))
saveRDS(hab_all, file.path(data_dir2, "Waikato_habitat_downstreamOnly.rds"))
saveRDS(obs_count, file.path(data_dir2, "Waikato_observations_downstreamOnly_count.rds"))
saveRDS(network_count, file.path(data_dir2, "Waikato_network_downstreamOnly_count.rds"))
saveRDS(hab_count, file.path(data_dir2, "Waikato_habitat_downstreamOnly_count.rds"))


		l2 <- lapply(1:nrow(network_all), function(x){
			parent <- network_all$parent_s[x]
			find <- network_all %>% filter(child_s == parent)
			if(nrow(find)>0) out <- cbind.data.frame(network_all[x,], 'long2'=find$long, 'lat2'=find$lat)
			if(nrow(find)==0) out <- cbind.data.frame(network_all[x,], 'long2'=NA, 'lat2'=NA)
			# if(nrow(find)>0) out <- cbind.data.frame(network_all[x,], 'long2'=find$long, 'lat2'=find$lat)
			# if(nrow(find)==0) out <- cbind.data.frame(network_all[x,], 'long2'=NA, 'lat2'=NA)
			return(out)
		})
...
catchmap <- ggplot() +
		geom_point(data=network_all, aes(x = long, y = lat), col="gray") +
		geom_segment(data=l2, aes(x = long2,y = lat2, xend = long, yend = lat), col="gray") +
		geom_point(data=obs_all, aes(x = long, y = lat, fill = data_type), pch=22, alpha=0.6) +
		scale_fill_brewer(palette = "Set1") +
		xlab("Longitude") + ylab("Latitude") +
		# guides(fill = FALSE) +
		theme_bw()
